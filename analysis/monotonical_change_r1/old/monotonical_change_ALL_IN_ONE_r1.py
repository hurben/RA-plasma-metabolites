...
def monotonic_change(file_i_df, file_i, output_dir, batch_num, fc_batch, label_list):
	...
	for FC in FC_list:
		...
		monotonic_dict, data_dict, _, chemID_list = calculation(file_i_df, FC)
		pvalue_dict = calculate_pvalue(file_i_df, monotonic_dict, FC)

		output_file_str = '%s/%s.monotonic_profile_%s.tsv' % (output_dir, output_str, FC)
		dict_to_text(output_file_str, monotonic_dict, data_dict, label_list, chemID_list, pvalue_dict)


def calculate_pvalue(file_i_df, monotonic_dict, FC):
	...
	print ('non dynamic: %s' % len(list(monotonic_dict.keys())))
	...
	for i in range(random_sampling):
		...
		random_monotonic_dict, _, _, _ = calculation(file_i_df, FC)

		for chemID in list(monotonic_dict.keys()):
			if chemID in list(random_monotonic_dict.keys()):
...
def calculation(file_i_df, FC):

	data_dict, label_list, chemID_list, num_labels = file_df_to_data_dict(file_i_df)
	...
	for chemID in chemID_list:
		...
		if monotonic_status != 'dynamic':
			monotonic_status_dict[chemID] = monotonic_status

	return monotonic_status_dict, data_dict, label_list, chemID_list
...
def file_df_to_data_dict(file_i_df):
	...
	return data_dict, label_list, chemID_list, num_labels
...
def main(data_location_dir, file_list, label_num, output_dir, batch_num, fc_batch):
	...
	for file_i in file_list:
		...
		monotonic_change(file_i_df, file_i, output_dir, batch_num, fc_batch, label_list)
...
if batch_num == 1:
	file_list = ['MLR.clp.fac.ready.ignor.norm.qc.fillna.tsv','MLR.clp.lc.ready.ignor.norm.qc.fillna.tsv','MLR.clp.sc.ready.ignor.norm.qc.fillna.tsv']
if batch_num == 2:
