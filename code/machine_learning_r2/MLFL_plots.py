class plots:
	...
	def plot_loo_mae_variance_plot(self, loo_mae_dict, summary_file, label_x, label_y):
		...
		if NUM_CONDITIONS == 9:
			color_list = sns.color_palette("RdBu_r", 10)
			color_list.pop(5)
			#my_color = sns.palplot(color_list)

		if NUM_CONDITIONS == 11:
			color_list = sns.color_palette("RdBu_r", 12)
			color_list.pop(5)
			#my_color = sns.palplot(color_list)
		...
		for i in range(len(list(loo_mae_dict.keys()))):
			...
			#highlighting colors that corresponds to our interest
			if folder == "hd4_only":
				#summary_sns_plot = sns.lineplot(set_index_list, loo_mae_list, label=folder, color="black")
				summary_sns_plot = sns.lineplot(set_index_list, loo_mae_list, label=folder)
				summary_sns_plot.lines[i].set_linestyle("--")
			if folder == "clp_lc_full":
				#summary_sns_plot = sns.lineplot(set_index_list, loo_mae_list, label=folder, color="green")
				summary_sns_plot = sns.lineplot(set_index_list, loo_mae_list, label=folder)
				summary_sns_plot.lines[i].set_linestyle("--")
			if folder != "hd4_only" and folder != "clp_lc_full":
	...
	#keys = observation, prediction
		...
		preobs_plot = sns.lineplot(theoretical_X, theoretical_y)
		preobs_plot = sns.regplot(data_point_dict["predict"], data_point_dict["observe"])
		preobs_plot.set(xlabel="prediction", ylabel="observation")
		preobs_plot.set_title(folder_name)
